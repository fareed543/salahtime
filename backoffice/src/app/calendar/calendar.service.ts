...
export class CalendarService {
  ...
  addSpecialDate(item: AdminCalendarSpecialDate): Observable<AdminCalendarSpecialDateResponse> {
    return this.getSpecialDates().pipe(
      map((response) => response.items ?? []),
      switchMap((items) => this.saveSpecialDatesCollection([...items, item]))
    );
  }

  updateSpecialDate(item: AdminCalendarSpecialDate): Observable<AdminCalendarSpecialDateResponse> {
    return this.getSpecialDates().pipe(
      map((response) => (response.items ?? []).map((current) => current.id === item.id ? item : current)),
      switchMap((items) => this.saveSpecialDatesCollection(items))
    );
  }

  deleteSpecialDate(id: number): Observable<AdminCalendarSpecialDateResponse> {
    return this.getSpecialDates().pipe(
  ...
  saveSpecialDatesCollection(items: AdminCalendarSpecialDate[]): Observable<AdminCalendarSpecialDateResponse> {
