...
import { AdminCalendarAdjustment, AdminCalendarSpecialDate, CalendarService } from './calendar.service';
...
@Component({
  selector: 'app-calendar-adjustments',
...
export class CalendarComponent {
  ...
  loadData(): void {
    ...
    this.calendarService.getAdjustments().subscribe({
      next: (adjustmentResponse) => {
        this.adjustmentItems = (adjustmentResponse.items ?? [])
        ...
        this.calendarService.getSpecialDates().subscribe({
          next: (specialDateResponse) => {
            this.specialDateItems = (specialDateResponse.items ?? [])
              .map((item) => ({ ...item }))
          ...
          error: (error) => {
            this.errorMessage = error?.error?.error || 'Unable to load calendar special dates right now.';
            this.isLoading = false;
          }
        });
      },
  ...
  addAdjustment(): void {
    const defaultYear = this.currentHijriYear;
    const newItem = this.hydrateAdjustment({
      id: Date.now(),
      title: '',
      hijriYear: defaultYear,
      hijriMonth: 1,
      originalStartDate: '',
      originalEndDate: '',
      updatedStartDate: '',
  ...
  addSpecialDate(): void {
    ...
    this.specialDateItems = [...this.specialDateItems, newItem].sort((left, right) => left.eventDate.localeCompare(right.eventDate) || (left.sortOrder - right.sortOrder));
  ...
  removeSelectedAdjustment(): void {
    ...
    this.selectedAdjustmentId = this.getPreferredSelectedAdjustmentId();
    this.feedbackMessage = 'Calendar month removed locally. Save changes to publish it.';
    this.errorMessage = '';
  }
    ...
    this.calendarService.saveAdjustments(payload).subscribe({
      next: (response) => {
        this.adjustmentItems = (response.items ?? [])
          ...
          .sort((left, right) => this.sortByHijriPeriod(right, left));
        this.selectedAdjustmentId = this.getPreferredSelectedAdjustmentId();
        this.isSavingAdjustments = false;
  ...
  saveSpecialDates(): void {
    ...
    this.isSavingSpecialDates = true;
    this.feedbackMessage = '';
    this.errorMessage = '';
    ...
    this.calendarService.saveSpecialDates(payload).subscribe({
      next: (response) => {
        this.specialDateItems = (response.items ?? [])
          .map((item) => ({ ...item }))
          .sort((left, right) => left.eventDate.localeCompare(right.eventDate) || (left.sortOrder - right.sortOrder));
        this.selectedSpecialDateId = this.specialDateItems[0]?.id ?? null;
        this.isSavingSpecialDates = false;
  ...
  private buildHijriYears(): number[] {
    const todayParts = this.getHijriParts(new Date());
