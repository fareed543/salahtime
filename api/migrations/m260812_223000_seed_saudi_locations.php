...
class m260812_223000_seed_saudi_locations extends Migration
{
    public function safeUp()
    {
        $seedPath = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . 'saudi-location-seed.json';
        if (!is_file($seedPath)) {
            throw new \RuntimeException('Saudi location seed file not found at ' . $seedPath);
        }

        $payload = json_decode((string)file_get_contents($seedPath), true);
        if (!is_array($payload)) {
            throw new \RuntimeException('Saudi location seed file is invalid JSON.');
        }
        ...
        if (!is_array($country) || !$states || !$cities) {
            throw new \RuntimeException('Saudi location seed file does not contain the required sections.');
        }

        $this->delete('{{%cities}}', ['country_id' => $this->fetchCountryIdBySlug('saudi-arabia')]);
        $this->delete('{{%states}}', ['country_id' => $this->fetchCountryIdBySlug('saudi-arabia')]);
        $this->delete('{{%countries}}', ['slug' => 'saudi-arabia']);
        ...
        if (!$countryId) {
            throw new \RuntimeException('Unable to create or resolve Saudi Arabia country record.');
        }
    ...
    {
        $countryId = $this->fetchCountryIdBySlug('saudi-arabia');
        if ($countryId) {
