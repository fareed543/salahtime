...
class m260812_233000_seed_pakistan_locations extends Migration
{
    public function safeUp()
    {
        $seedPath = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . 'pakistan-location-seed.json';
        if (!is_file($seedPath)) {
            throw new \RuntimeException('Pakistan location seed file not found at ' . $seedPath);
        }

        $payload = json_decode((string)file_get_contents($seedPath), true);
        if (!is_array($payload)) {
            throw new \RuntimeException('Pakistan location seed file is invalid JSON.');
        }
        ...
        if (!is_array($country) || !$states || !$cities) {
            throw new \RuntimeException('Pakistan location seed file does not contain the required sections.');
        }

        $this->delete('{{%cities}}', ['country_id' => $this->fetchCountryIdBySlug('pakistan')]);
        $this->delete('{{%states}}', ['country_id' => $this->fetchCountryIdBySlug('pakistan')]);
        $this->delete('{{%countries}}', ['slug' => 'pakistan']);
        ...
        if (!$countryId) {
            throw new \RuntimeException('Unable to create or resolve Pakistan country record.');
        }
    ...
    {
        $countryId = $this->fetchCountryIdBySlug('pakistan');
        if ($countryId) {
