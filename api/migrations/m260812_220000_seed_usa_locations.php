...
class m260812_220000_seed_usa_locations extends Migration
{
    public function safeUp()
    {
        $seedPath = dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . 'usa-location-seed.json';
        if (!is_file($seedPath)) {
            throw new \RuntimeException('USA location seed file not found at ' . $seedPath);
        }

        $payload = json_decode((string)file_get_contents($seedPath), true);
        if (!is_array($payload)) {
            throw new \RuntimeException('USA location seed file is invalid JSON.');
        }
        ...
        if (!is_array($country) || !$states || !$cities) {
            throw new \RuntimeException('USA location seed file does not contain the required sections.');
        }

        $this->delete('{{%cities}}', ['country_id' => $this->fetchCountryIdBySlug('united-states')]);
        $this->delete('{{%states}}', ['country_id' => $this->fetchCountryIdBySlug('united-states')]);
        $this->delete('{{%countries}}', ['slug' => 'united-states']);
        ...
        if (!$countryId) {
            throw new \RuntimeException('Unable to create or resolve United States country record.');
        }
    ...
    {
        $countryId = $this->fetchCountryIdBySlug('united-states');
        if ($countryId) {
