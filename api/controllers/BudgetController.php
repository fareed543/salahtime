        ...
        if ($headers->has('Authorization')) {
            ...
            if ($user) {
                ...
                $response = [
                    ...
                    'categoryImagePath' => Yii::$app->params['categoryImagePath'],
                    'expenseImagePath' => Yii::$app->params['expenseImagePath'],
            ...
            if ($user) {
                ...
                if ($imageFile) {
                    ...
                    if (in_array($imageFileExtension, $imageExtensions)) {
                        ...
                        $savedImageName = $compressedImageName;
                ...
                if (!empty($transactions)) {
                    ...
                    if (!is_array($transactions)) {
                        Yii::$app->response->statusCode = 400;
                        return \yii\helpers\Json::encode(['error' => 'Invalid transactions payload']);
                    }
                }
                ...
                $rowsToSave = [];
                if (!empty($transactions)) {
                    $rowsToSave = $transactions;
                } else {
                    $rowsToSave[] = [
                        'name' => Yii::$app->request->post('name'),
                        'category' => Yii::$app->request->post('category'),
                        'amount' => Yii::$app->request->post('amount'),
                        'dateOfTransaction' => Yii::$app->request->post('dateOfTransaction'),
                ...
                foreach ($rowsToSave as $row) {
                    ...
                    $newExpense->id_category = $row['category'] ?? null;
                    $newExpense->expense_name = $row['name'] ?? null;
                    ...
                    if (!empty($row['serialNo'])) {
                        $descriptionParts[] = 'S No: ' . $row['serialNo'];
                    }
                    if (isset($row['quantity']) && $row['quantity'] !== '') {
                        $descriptionParts[] = 'Qty: ' . $row['quantity'];
                    }
                    if (isset($row['unitPrice']) && $row['unitPrice'] !== '') {
                        $descriptionParts[] = 'Unit Price: ' . $row['unitPrice'];
                    }
                    $newExpense->description = implode(' | ', $descriptionParts);
                    $newExpense->amount = $row['amount'] ?? null;

                    $dateOfTransaction = DateTime::createFromFormat('Y-m-d', $row['dateOfTransaction'] ?? '');
                    ...
                    if ($savedImageName) {
                        $newExpense->image = $savedImageName;
                    }

                    if ($id_event !== null && $id_event !== '' && $id_event !== 'null') {
                    ...
                    $savedExpenses[] = $newExpense;
                }

                Yii::$app->response->statusCode = 201;
                return \yii\helpers\Json::encode([
                    'count' => count($savedExpenses),
                    'items' => $savedExpenses
                ]);
            } else {
                Yii::$app->response->statusCode = 401;
